...
from terapy_2.hardware.input.base import InputDevice
...
if _support_PXI:
    ...
    class PXI5122(InputDevice):
        ...
        def read(self):    
            ...
            self.pxi.start()
            startT = time()
            while(self.pxi.completed() != 1 and time() < startT + PXI_TIMEOUT):
                sleep(0.01)
            if(time() > startT + PXI_TIMEOUT):
                return None
...
#                        valueCH1 = (dataCH1[NS * i + 0] + dataCH1[NS * i + 1] + dataCH1[NS * i + 2] + dataCH1[NS * i + 3] + dataCH1[NS * i + 4]) / 5                
    ...
    class PXI5122LockIn(InputDevice):
        ...
        def send_configuration(self):
            ...
            self.pxi.config_time(0)    # when clocking to the 80MHz laser, the frequencies are not valid any more        
        ...
        def read(self):    
            ...
            self.pxi.start()
            startT = time()
            while(self.pxi.completed() != 1 and time() < startT + self.inttime * 1e-3 + PXI_TIMEOUT):
                sleep(0.01)
            if(time() > startT + PXI_TIMEOUT):
                return None
